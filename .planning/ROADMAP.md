...
- [x] **Phase 2: Short-Form, Supra & Ambiguous-Match Resolution** - Port short-form/supra/ambiguous-match tests and fix the caseNamesMatch/hallucinationCheck bugs they expose (completed 2026-07-15)
- [x] **Phase 3: Case-Name & HTML Safety, Full Traceability** - Port HTML-escaping/case-name-safety tests and finalize the dedicated ported-test file across all four categories (completed 2026-07-15)
...
- [x] 01-01-PLAN.md — Port CourtListener reporter-spacing tests (RED) and fix the un-normalized `reporter` field in `parseCaseCitation` via a benchmarked `normalizeReporterSpacing` helper (GREEN)
...
- [x] 02-02-PLAN.md — Port ambiguous-match tests (RED) and fix `courtListenerProvider.lookupCitation`'s silent multi-cluster collapse via `caseNamesMatch` disambiguation + a new `ambiguousMatch` field propagated as a distinct outcome through `hallucinationCheck` (GREEN) [TEST-04, FIX-02]
...
- [x] 03-01-PLAN.md — Port CourtListener case-name/HTML-escaping safety tests (TEST-05) against the four in-scope functions, add the missing `stripHtmlHyperlinks` double-unescape regression test, and perform the whole-file TEST-06 traceability closeout (confirm-and-port: RESEARCH found no bug)
...
| 3. Case-Name & HTML Safety, Full Traceability | 1/1 | Complete    | 2026-07-15 |
