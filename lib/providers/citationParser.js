...
exports.extractCitationTokens = extractCitationTokens;
exports.caseNamesMatch = caseNamesMatch;
exports.clusterCitationTokens = clusterCitationTokens;
exports.findOrphanedCitations = findOrphanedCitations;
exports.extractCaseCitations = extractCaseCitations;
...
const CASE_CITATION_REGEX = new RegExp(`${CASE_NAME}\\s+v\\.?\\s+${CASE_NAME},\\s*${NUMBER}\\s+[A-Za-z0-9.&' ]+?\\s+${NUMBER}${PINCITE}(?:\\s*\\([^)]*\\))?`, "g");
...
// token pattern.
const SHORT_FORM_CITATION_REGEX = new RegExp(`${CASE_NAME},\\s*${NUMBER}\\s+[A-Za-z0-9.&' ]+?\\s+at\\s+${PINCITE_LIST}`, "g");
...
const LEADING_SIGNAL_REGEX = /^(?:see\s*,?\s*e\.g\.,?|see\s+also|see\s+generally|but\s+see|but\s+cf\.?|cf\.?|accord|contra|compare|see|in(?!\s+re\b))\s+/i;
...
const SHORT_FORM_REGEX = new RegExp(`(?:(${CASE_NAME}),\\s+)?(${NUMBER})\\s+([A-Za-z0-9.&' ]{1,40}?)\\s+at\\s+(${PINCITE_PAGE})`, "g");
...
const SUPRA_REGEX = new RegExp(`(${CASE_NAME}),\\s+[Ss]upra(?:,?\\s+at\\s+(${PINCITE_PAGE}))?(?:,?\\s+note\\s+(\\d+))?`, "g");
...
function matchAllToArray(text, regex) {
...
function extractCitationTokens(text) {
    ...
    for (const match of matchAllToArray(text, CASE_CITATION_REGEX)) {
        if (match.index === undefined)
            continue;
        const raw = (0, utils_1.normalizeText)(match[0]).replace(LEADING_SIGNAL_REGEX, "");
        if (!raw)
            continue;
        fullSpans.push([match.index, match.index + match[0].length]);
        tokens.push({ type: "full", raw, index: match.index });
    }
    const withinFullSpan = (index) => fullSpans.some(([start, end]) => index >= start && index < end);
    ...
    const normalizeNamePart = (namePart) => (0, utils_1.normalizeText)(namePart).replace(LEADING_SIGNAL_REGEX, "");
    for (const match of matchAllToArray(text, SHORT_FORM_REGEX)) {
        if (match.index === undefined || withinFullSpan(match.index))
            continue;
        const [, namePart, , , pincite] = match;
        tokens.push({
            type: "short",
            raw: (0, utils_1.normalizeText)(match[0]),
            index: match.index,
            namePart: namePart ? normalizeNamePart(namePart) : undefined,
            pincite: pincite ? (0, utils_1.normalizeText)(pincite) : undefined,
        });
    }
    for (const match of matchAllToArray(text, ID_REGEX)) {
        ...
        tokens.push({
            ...
            raw: (0, utils_1.normalizeText)(match[0]),
            index: match.index,
            pincite: match[1] ? (0, utils_1.normalizeText)(match[1]) : undefined,
        });
    }
...
function caseNameMatchesToken(caseName, namePart) {
...
function normalizeCaseNameParty(party) {
    return party
        .toLowerCase()
        .replace(/[.,]/g, "")
        .replace(/\s+/g, " ")
...
function partyWordsContain(container, contained) {
...
function caseNamesMatch(a, b) {
    ...
    const partyMatches = (p1, p2) => (p1 !== "" && p1 === p2) || partyWordsContain(p1, p2) || partyWordsContain(p2, p1);
 ...
 */
function clusterCitationTokens(tokens) {
    const clusters = [];
    let lastCluster = null;
    for (const token of tokens) {
        if (token.type === "full") {
            const parsed = parseCaseCitation(token.raw);
            const cluster = { leadCitation: token.raw, caseName: parsed === null || parsed === void 0 ? void 0 : parsed.caseName, tokens: [token] };
        ...
        if (token.namePart) {
            ...
            target = lastCluster !== null && lastCluster !== void 0 ? lastCluster : undefined;
        }
        if (target) {
 ...
 */
function findOrphanedCitations(text) {
 ...
 */
function extractCaseCitations(text) {
    const matches = [
    ...
    matches.forEach((match) => {
        const citation = (0, utils_1.normalizeText)(match).replace(LEADING_SIGNAL_REGEX, "");
        if (citation) {
            unique.add(citation);
        }
    });
 ...
 */
function parseCaseCitation(text) {
    const raw = (0, utils_1.normalizeText)(text);
    if (!raw) {
        return null;
    }
    ...
    const match = raw.match(new RegExp(`^(.+?),\\s*(${NUMBER})\\s+([A-Za-z0-9.&' ]+?)\\s+(?<!\\bat\\s)(${NUMBER})(?:,\\s*(${PINCITE_LIST}))?\\s*(?:\\(([^)]*)\\))?\\s*$`));
    if (match) {
        const [, caseName, volume, reporter, page, pincite, parenthetical] = match;
        const parsed = {
            raw,
            caseName: caseName === null || caseName === void 0 ? void 0 : caseName.trim(),
            volume: volume === null || volume === void 0 ? void 0 : volume.trim(),
            reporter: reporter === null || reporter === void 0 ? void 0 : reporter.trim(),
            page: page === null || page === void 0 ? void 0 : page.trim(),
        };
        if (pincite) {
    ...
    const shortMatch = raw.match(new RegExp(`^(${CASE_NAME}),\\s*(${NUMBER})\\s+([A-Za-z0-9.&' ]+?)\\s+at\\s+(${PINCITE_LIST})\\s*$`));
    if (shortMatch) {
        ...
        return {
            ...
            volume: volume === null || volume === void 0 ? void 0 : volume.trim(),
            reporter: reporter === null || reporter === void 0 ? void 0 : reporter.trim(),
            pincite: pincite === null || pincite === void 0 ? void 0 : pincite.trim(),
            isShortForm: true,
        };
    }
